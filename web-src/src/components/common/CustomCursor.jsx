...
const GLITCH_GLYPHS_IT = ['0', '1', '[', ']', '{', '}', ';', '=>', '++', '--', '&&', '||', '!', '$', '?', '#', 'x', 'y', 'f', 'a'];
const GLITCH_GLYPHS_ADMIN = ['SUM', 'IF', 'QTY', 'A1', 'B2', 'C3', '$', '%', '=', '*', '+', '-', '/'];

const SYMBOLS_IT = ['</>', '{ }', '=>', ';', 'const', 'dev', '[]', '&&', '||', '++', '()', '$'];
const SYMBOLS_ADMIN = ['fx()', '[xls]', '[csv]', '[doc]', '[pdf]', 'sum', 'if', '=', '$', 'qty', 'stock', 'data'];

const CODE_KEYWORDS_IT = [
...
export default function CustomCursor({ isAdmin = false }) {
  ...
  useEffect(() => {
    ...
    if (!cur || !ring || !trailContainer) return;
    ...
    const SYMBOLS = isAdmin ? SYMBOLS_ADMIN : SYMBOLS_IT;
    ...
    // Keyword emission trail
    const trailWords = [];
    const CODE_KEYWORDS = isAdmin ? CODE_KEYWORDS_ADMIN : CODE_KEYWORDS_IT;
    let lastEmitX = mouseX;
    let lastEmitY = mouseY;
    const EMIT_DISTANCE = 35;
    ...
    // Glitch particles on click
    const glitchParticles = [];
    const GLITCH_GLYPHS = isAdmin ? GLITCH_GLYPHS_ADMIN : GLITCH_GLYPHS_IT;

    let scrambleTimeout, compileTimeout, normalTimeout;
    ...
    function updateAnimation() {
      ...
      // 3. Emit keywords based on movement distance
      ...
      // 4. Move and Scramble/Compile segments
    ...
    const handleMouseOver = (e) => {
      ...
      if (target) {
        ...
        segments.forEach(seg => seg.el.classList.add('hovered'));
      }
    };
    ...
    const handleMouseOut = (e) => {
      ...
      if (target) {
        ...
        segments.forEach(seg => seg.el.classList.remove('hovered'));
      }
    };
  ...
  return (
    <>
      ...
      <div id="cursorTrailContainer" ref={trailContainerRef} style={{
        position: 'fixed',
        top: 0,
        left: 0,
        width: '100vw',
