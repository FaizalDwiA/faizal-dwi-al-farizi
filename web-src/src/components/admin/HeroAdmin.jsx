...
export default function HeroAdmin({ projectCount = 0, certificateCount = 0 }) {
  const { badge, title, roleCmd, desc, hud, terminal } = adminData.hero;
      ...
      <div className="hero-inner">
        <div className="hero-text fade-in visible">
          ...
          <div className="hero-ctas">
            <button onClick={() => document.getElementById('services')?.scrollIntoView({ behavior: 'smooth' })} className="btn-primary">▶ Lihat Spesialisasi</button>
            <button onClick={() => document.getElementById('projects')?.scrollIntoView({ behavior: 'smooth' })} className="btn-outline">📁 Proyek & Kerja</button>
          </div>
          <div className="hero-stats">
            <div className="stat">
              ...
              <div className="stat-lbl">ADMIN & IT PROJECTS</div>
            </div>
            <div className="stat">
              <div className="stat-num">1000+</div>
              <div className="stat-lbl">DATA RECORDS</div>
            </div>
            <div className="stat">
              <div className="stat-num">
            ...
            {!isDiagnosing ? (
              <div id="termContent">
                ...
                <div className="t-indent" style={{ paddingLeft: '3rem' }}>
                  <span className="t-key">"spreadsheets"</span>: [
                  {terminal.profile.skills.spreadsheets.map((s, i) => (
                    <span key={i}>
                      <span className="t-str">"{s}"</span>
                      {i < terminal.profile.skills.spreadsheets.length - 1 ? ', ' : ''}
                    </span>
                  ))}
                  ],
                  <br />
                  <span className="t-key">"it_support"</span>: [
                  {terminal.profile.skills.it_support.map((h, i) => (
                    <span key={i}>
                      <span className="t-str">"{h}"</span>
                      {i < terminal.profile.skills.it_support.length - 1 ? ', ' : ''}
                    </span>
                  ))}
                  ]
                </div>
