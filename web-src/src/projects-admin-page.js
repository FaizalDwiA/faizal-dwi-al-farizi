...
import { fetchAdminProjects } from './firebase/firestore.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Force redirect back to admin pages (role check is admin here)
  ...
  // Helper function to format Google Drive direct links
  ...
  if (termBody) {
    stepTimeout1 = setTimeout(() => {
      ...
      line.innerHTML = '<span style="color:#66d9ef">[PARSING]</span> Memetakan data & formula sheet... <span class="skeleton-terminal-cursor"></span>';
  ...
  let projectsList = [];
  try {
    const projectsRes = await fetchAdminProjects();
  ...
  const mappedProjects = projectsList.map(p => {
    ...
    let icon = 'bi bi-file-earmark-spreadsheet';
    ...
    const link = `project-details-admin.html?id=${p.id}`;
  ...
  function renderProjects() {
    const filtered = mappedProjects.filter(p => {
      ...
      return !searchQuery || textToSearch.includes(searchQuery);
    });
    ...
    filtered.forEach((p, i) => {
      ...
      // Render formulas/tech badge lists
      ...
      projGrid.innerHTML += `
      <div class="proj-card fade-in" style="transition-delay:${delay}ms">
        <div class="proj-type">${p.category || 'Excel / Admin'}</div>
